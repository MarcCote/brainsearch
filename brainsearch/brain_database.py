...
class NewBrainDatabase(object):
    ...
    def labels_count(self, check_integrity=False):
        ...
        labels_count = [info["label_count_0"], info["label_count_1"]]
    ...
    def update(self, nb_patches=None, labels_count=None):
        ...
        if labels_count is not None:
            for i, count in enumerate(labels_count):
                info["label_count_{0}".format(i)] += count
...
class NewBrainDatabaseManager(object):
    DATABASES_LIST_KEY = "BRAIN_DB"

    def __init__(self, storage_type, **storage_params):
        self.storage_type = storage_type
        self.storage_params = storage_params
        self.storage = storage_factory(storage_type, **storage_params)
        self.brain_databases = {}

        #Retrieves existing brain databases
        names = self.storage.get_info(NewBrainDatabaseManager.DATABASES_LIST_KEY)
        for name in names:
            ...
            brain_database = NewBrainDatabase(name, self.storage, engine)
            self.brain_databases[name] = brain_database
